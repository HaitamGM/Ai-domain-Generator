...
#     Check if a domain is available using the python-whois library.
#     A domain is considered available if the WHOIS query has no creation date
#     or if the server explicitly returns a 'No match' response.
#     This function is cached using LRU cache.
#     """
#     try:
#         w = whois.whois(domain)
#         # A None creation_date is a strong indicator of availability.
#         return w.creation_date is None
#     except Exception as e:
#         # If the exception's text contains "No match for", it's definitely available.
#         if "No match for" in str(e):
#             return True  # This is a success case, not an error.
#
#         # For all other exceptions (e.g., network errors), log a warning and assume
#         # availability as a fallback.
#         app.logger.warning(f"Error checking domain {domain}: {str(e)}. Assuming available.")
#         return True
#
#
...
#
#         # Check all domains in parallel with higher concurrency
#         availability_results = check_domains_parallel(all_domains_to_check, max_workers=5)
#
#         # Filter to only available domains
#         available_domains = []
#
...
STYLE_PROMPTS = {
    ...
    "moroccan": (
        ...
        "Examples: 'SoukTech', 'DarBusiness', 'AtlasShop', 'SahaServices'. 'AtayEssence' "
        "Blend Moroccan culture naturally with the business concept. "
        "Use authentic Moroccan terms and cultural references."
        "Do not force any darija elements/words within the name if it doesn't feel natural and makes sense"
    ),
    "professional": (
        "Create formal, corporate-style domains suitable for B2B and enterprise. "
        "Use terms like: Solutions, Global, Pro, Enterprise, Systems, Group. "
...
def generate_enhanced_fallback_domains(idea, style="default", extensions=None, n=40):
    if not extensions:
        extensions = ['.com', '.ma']
...
def generate_fallback_domains(idea, style="default", extensions=None, n=40):
    return generate_enhanced_fallback_domains(idea, style, extensions, n)
